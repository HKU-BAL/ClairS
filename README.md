...
## Latest Updates
*v0.5.1 (Sep 10, 2026)* : **Infrastructure and compatibility update.** 1. Rebuilt the Docker image on `condaforge/miniforge3` with Python 3.11 and Clair3 2.0.3, replacing the `ubuntu:16.04` base, whose glibc 2.23 can no longer support current dependencies (Miniconda and Clair3 2.0.x both require glibc >= 2.28) and whose apt sources are EOL. The build host now requires Docker >= 20.10.10. 2. Added the `ont_r10_dorado_hac_5khz_v6` platform (`-p ont_r10_dorado_hac_5khz_v6`), which pairs the ClairS v6 model with the Clair3 `r1041_e82_400bps_hac_v600` germline model. 3. Added `--gender` and `--par_regions_bed` to support sex-chromosome ploidy-aware germline calling, forwarded to Clair3 ([#66](https://github.com/HKU-BAL/Clair3/issues/66)). 4. Replaced shell calls to `gzip` with `pigz`, so decompression no longer depends on an undeclared `gzip` ([#455](https://github.com/HKU-BAL/Clair3/issues/455)). 5. A failed BAM/CRAM decoding now exits with a non-zero status instead of silently producing an empty VCF ([#453](https://github.com/HKU-BAL/Clair3/issues/453)).
...
|           ONT           | r1041_e82_400bps_hac_v600 | R10.4.1, 5khz | Dorado HAC |  `ont_r10_dorado_hac_5khz_v6`  | GRCh38_no_alt | Minimap2 |
...
docker run -it \
  ...
  --platform ${PLATFORM} \                     ## options: {ont_r10_dorado_sup_4khz, ont_r10_dorado_sup_5khz, ont_r10_dorado_hac_5khz_v6, ont_r10_guppy, ont_r9_guppy, ilmn, hifi_sequel2, hifi_revio, etc}
...
```bash
  ...
  -p, --platform PLATFORM           Select the sequencing platform of the input. Possible options {ont_r10_dorado_sup_4khz, ont_r10_dorado_sup_5khz, ont_r10_dorado_hac_5khz_v6, ont_r10_dorado_hac_5khz, ont_r10_dorado_hac_4khz, ont_r10_guppy, ont_r9_guppy, ilmn, hifi_sequel2, hifi_revio}.
```
