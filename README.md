...
## Latest Updates
*v0.5.1 (Sep 9, 2026)* : 1. Added the `ont_r10_dorado_hac_5khz_v6` platform (`-p ont_r10_dorado_hac_5khz_v6`), which enables ONT Dorado HAC 5kHz v6 model. 2. Rebuilt the Docker image with the latest Clair3 version and model. The image is now built on `condaforge/miniforge3` with Python 3.11 and Clair3 2.0.3, replacing the `ubuntu:16.04` base, whose glibc 2.23 can no longer support current dependencies. The build host now requires Docker >= 20.10.10. 3. Added `--gender` and `--par_regions_bed` to support sex-chromosome ploidy-aware germline calling, forwarded to Clair3 ([#66](https://github.com/HKU-BAL/Clair3/issues/66)). 4. Replaced shell calls to `gzip` with `pigz`, so decompression no longer depends on an undeclared `gzip` ([#455](https://github.com/HKU-BAL/Clair3/issues/455)).
