...
def write_table_dict(table_dict, normal_matrix, tumor_matrix, label, pos, total, normal_alt_info, tumor_alt_info,
    ...
    if apply_normalize:
        ...
        if normal_coverage == 0 or tumor_coverage == 0:
            return total
        for idx in range(param.no_of_positions):
            input_matrix += [float(item) / normal_coverage for item in
        ...
        for idx in range(param.no_of_positions):
            input_matrix += normal_matrix[idx * channel_size: (idx + 1) * channel_size]
            input_matrix += tumor_matrix[idx * tumor_channel_size: (idx + 1) * tumor_channel_size]

    if len(input_matrix) != param.no_of_positions * (channel_size + tumor_channel_size):
        return total
...
def write_table_file(table_file, table_dict, tensor_shape, label_size, float_type):
    ...
    table_file.root.proportion.append(np.array(table_dict['proportion'], np.dtype(float_type)).reshape(-1, 1))
...
def get_training_array(args,
    ...
    tensor_shape = [param.no_of_positions, channel_size + param.tumor_channel_size]  # normal and tumor
    ...
    for normal_tensor_fn, tumor_tensor_fn in zip(normal_tensor_list, tumor_tensor_list):
        ...
        proportion = tumor_tensor_fn.split("_")[-1]
        ...
        for X, batch_total in heapq_merge_generator_from(normal_bin_reader_generator, tumor_bin_reader_generator):
            ...
            for key, normal_index, tumor_index in all_pos_list:
                ...
                if add_af_in_label:
                    ...
                    max_normal_af = 0
                    max_tumor_af = 0
                    normal_depth, normal_alt = nomral_alt_info.split('-')[:2]
                    ...
                    for tumor_alt, tumor_count in tumor_alt_dict.items():
                        ...
                        tumor_af = tumor_count / float(tumor_depth)
                        normal_count = normal_alt_dict[tumor_alt] if tumor_alt in normal_alt_dict else 0
                        normal_af = normal_count / float(normal_depth) if float(normal_depth) > 0 else 0.0
                        # if tumor_af - normal_af > 0:
                        support_alt_dict[tumor_alt] = [tumor_af - normal_af, tumor_af, normal_af]
                    ...
                    alt_type_list = sorted(support_alt_dict.items(), key=lambda x: x[1][0], reverse=True)
                    if len(alt_type_list) == 0:
                        print(' not found', normal_alt_dict, tumor_alt_dict, label)
                        continue
                    best_match_alt, best_af_list = alt_type_list[0]
                    af_gap, tumor_af, normal_af = best_af_list
                    ...
                    if af_gap is None or tumor_af is None or normal_af is None:
                        print('skip')
                        continue
                    label += [af_gap, normal_af, tumor_af]
                    # print(label)
